...
bool EditShapeOP::OnDraw(const ur::Device& dev, ur::Context& ctx) const
{
	...
	if (m_proxy_op) {
		return m_proxy_op->OnDraw(dev, ctx);
	}

	if (m_op_state) {
		m_op_state->OnDraw(dev, ctx);
	}
...
void EditShapeOP::ChangeEditState(uint32_t shape_type, std::shared_ptr<gs::Shape2D> selected)
{
    ...
    } else if (shape_type == rttr::type::get<gs::Line2D>().get_id()) {
        ChangeEditOpState(std::make_shared<EditLineState>(m_camera, m_view, get_selected));
